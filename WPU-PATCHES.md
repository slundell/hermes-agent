...
## DEPLOYED + PROMOTED (2026-06-23)

`wpu-lcm-plugin` rebuilt onto **v2026.6.19** (= `sync/v2026.6.19`, 16 CARRY patches), deployed live
(`hermes-agent 0.17.0`), and **validated in vivo** (session 20260623_210237: 94–100% prefix cache,
MCP/terminal/file tools working, no regressions; P1 confirmed — LCM loads config-only).
Pre-sync state preserved at tag `archive/wpu-lcm-plugin-pre-sync-20260623` (commit 1b7f088b1).
Rollback: `git reset --hard archive/wpu-lcm-plugin-pre-sync-20260623` + `pip install -e` + gateway restart.
